...
public class ContentRegex {
    ...
    private static final String URL_VALID_SUBDOMAIN = "(?>(?:" + URL_VALID_CHARS
            + "[" + URL_VALID_CHARS + "\\-_]*)?" + URL_VALID_CHARS + "\\.)";
    private static final String URL_VALID_DOMAIN_NAME = "(?:(?:" + URL_VALID_CHARS
     ...
     **/
    private static final String URL_VALID_PATH_ENDING_CHARS = "[a-z" + CYRILLIC_CHARS
  ...
  public static final int VALID_MENTION_GROUP_BEFORE = 1;
    ...
    // initializing in a static synchronized block, there appears to be thread safety issues with
    // Pattern.compile in android
    static {
        synchronized(ContentRegex.class) {
            RTL_CHARACTERS =
                    Pattern.compile("[\u0600-\u06FF\u0750-\u077F\u0590-\u05FF\uFE70-\uFEFF]");
            AT_SIGNS = Pattern.compile("[" + AT_SIGNS_CHARS + "]");
            HASH_SIGNS = Pattern.compile("[" + HASH_SIGNS_CHARS + "]");

            VALID_MENTION = Pattern.compile("([^a-z0-9_!#$%&*/" + AT_SIGNS_CHARS
                    + "]|^|(?:^|[^a-z0-9_+~.-])RT:?)(" + AT_SIGNS + "+)([a-z0-9_]{1,15})?",
                    Pattern.CASE_INSENSITIVE);

            VALID_HASH = Pattern.compile("([^a-z0-9_!#$%&*/" + HASH_SIGNS_CHARS
                    + "]|^|(?:^|[^a-z0-9_+~.-])RT:?)(" + HASH_SIGNS + "+)([a-z0-9_]{1,15})?",
                Pattern.CASE_INSENSITIVE);

            VALID_EMOTICON = Pattern.compile("(\\()([a-z0-9]{1,15})(\\))",
                    Pattern.CASE_INSENSITIVE);
