...
import com.crossover.crosschat.android.core.util.ContentRegex;
...
public class ContentRegexTests {
    ...
    public void testExtractMentions() {
        assertCaptureCount(3, ContentRegex.VALID_MENTION,
                "sample @user mention");
    }

    @Test
    public void testExtractHashTags() {
        assertCaptureCount(3, ContentRegex.VALID_HASH,
            "sample #user mention");
    }
    ...
    public void testInvalidMentions() {
        ...
        for (char c : invalidChars) {
            assertFalse("Failed to ignore a mention preceded by " + c,
                    ContentRegex.VALID_MENTION.matcher("f" + c + "@kn").find());
        }
    }
    ...
    public void testHashTagsRegex() {
        assertCaptureCount(3, ContentRegex.VALID_HASH, "#hashtags");
    }

    @Test
    public void testExtractEmoticons() {
        assertCaptureCount(3, ContentRegex.VALID_EMOTICON,
                "sample (emoticon) within message");
    }

    @Test
    public void testValidURL() {
        assertCaptureCount(8, ContentRegex.VALID_URL,
                "http://example.com");
        assertCaptureCount(8, ContentRegex.VALID_URL,
                "http://はじめよう.みんな");
        assertCaptureCount(8, ContentRegex.VALID_URL,
    ...
    public void testValidURLDoesNotCrashOnLongPaths() {
        String longPathIsLong =
                "Check out http://example.com/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" +
        ...
        assertTrue("Failed to correctly match a very long path",
                ContentRegex.VALID_URL.matcher(longPathIsLong).find());
    }
    ...
    public void testValidUrlDoesNotTakeForeverOnRepeatedPunctuationAtEnd() {
        ...
        for (String text : repeatedPaths) {
            ...
            assertTrue(
                    "Should be able to extract a valid URL even followed by punctuations",
                    isValid);
    ...
    public void testValidUrlDoesNotOverflowOnLongDomains() {
        ...
        assertTrue("Match a super long url",
                ContentRegex.VALID_URL.matcher(domainIsLong).matches());
    }
    ...
    private void assertCaptureCount(int expectedCount, Pattern pattern, String sample) {
        ...
        assertEquals("Does not have " + expectedCount + " captures as expected: '"
                        + sample + "'",
                expectedCount,
